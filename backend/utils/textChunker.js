...
export const findRelevantChunks = (chunks, query, maxChunks = 3) => {
  ...
  const filteredChunks = scoredChunks.filter((chunk) => chunk.score > 0 || chunk.rawScore > 0);

  if (filteredChunks.length === 0) {
    // Fallback: If no chunks strictly match the query (e.g., query is "summarize this pdf"),
    // return the first few chunks so the AI still has context to work with.
    return chunks.slice(0, maxChunks).map((chunk) => ({
      content: chunk.content,
      chunkIndex: chunk.chunkIndex,
